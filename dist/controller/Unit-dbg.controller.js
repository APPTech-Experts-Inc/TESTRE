sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return Controller.extend("com.apptech.realestate.controller.Unit", {
		...
		prepareTable: function (bIsInit) {
			$.ajax({
				url: "/rexsjs/public/rexsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GETUNITDATA&keyCode=&queryType=GetAllUnit",
				type: "GET",
				xhrFields: {
					withCredentials: true
				},
		...
		onEdit: function (oEvent) {
			...
			var sQueryType = "GetUnitSpecific";
			var sCode = "";
			if (iIndex !== -1) {
				...
				//AJAX selected Key
				$.ajax({
					url: "/rexsjs/public/rexsjs/ExecQuery.xsjs?dbName=APP_RE&procName=SPAPP_RE_GETUNITDATA&keyCode=" +
		...
		onSave: function (oEvent) {
			...
			if (this.bIsAdd === "A") {
				...
				$.ajax({
					...
					if (results) {
						...
						if (this.resultAjaxCall === 0) {
							MessageToast.show("Saved Successfully " + this.oMdlEditRecord.getData().EditRecord.UnitDesc);
							this.onClearAdd();
						} else {
							MessageToast.show("Error");
				...
				this.dataObject.UnitCode = this.oMdlEditRecord.getData().EditRecord.UnitCode;
				this.dataObject.UnitDesc = this.oMdlEditRecord.getData().EditRecord.UnitDesc;
				this.dataObject.LotNo = this.oMdlEditRecord.getData().EditRecord.LotNo;
				this.dataObject.PhaseNo = this.oMdlEditRecord.getData().EditRecord.PhaseNo;
				this.dataObject.BlockNo = this.oMdlEditRecord.getData().EditRecord.BlockNo;
